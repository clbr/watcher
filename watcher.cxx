...
void tick(void* v){
 checkup();
 if (batname[0]!='\0') batcheckup();
 Fl_Box* box=(Fl_Box*)v;
 char yeah[45]="";
 if(showswap){
	if(mib) sprintf(yeah,"C%.1f%% M%.1f%s  S%.1f%s%s",cpu,mem,memind,swap,swapind,batmsg);
	else sprintf(yeah,"%.1f%%C  %.1f%%M  %.1f%%S%s",cpu,mem,swap,batmsg);
 }
 else{
	if(mib) sprintf(yeah,"C%.1f%% M%.1f%s%s",cpu,mem,memind,batmsg);
	else sprintf(yeah,"%.1f%%C  %.1f%%M%s",cpu,mem,batmsg);
 }
 box->copy_label(yeah);
